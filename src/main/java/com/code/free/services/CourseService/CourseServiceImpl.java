...
public class CourseServiceImpl implements CourseService {
        ...
        public ApiResult<String> addVideosToCourse(List<VideoCreateRequest> videoRequests, Long courseId)
                ...
                return CustomResponse.success("Videos added successfully", "Videos added successfully",
                                HttpStatus.CREATED);
        }
        ...
        public ApiResult<ViewCourseResponseDto> viewCourse(Long courseId) {
                ...
                UserCourseMappingEntity userCourseMappingResponse = userCourseMappingRepo
                                .findByUserIdAndCourseId(currentUser.getId(), course.getId()).orElse(null);
                ...
                if (userCourseMappingResponse == null) {
                        ...
                        courseResponse = ViewCourseResponseDto.builder()
                                        ...
                                        .courseDiscount(course.getDiscount())
                                        .createdBy(course.getCreatedBy())
                                        .videosList(videoService.videosForCourseWithPresignedUrl(courseId).getBody()
                                                        .getData())
                                        .build();
                }
        ...
        public ApiResult<String> purchaseCourse(Long courseId) {
                UserEntity currentUser = utils.getCurrentUser();
                CourseEntity course = courseRepository.findCourseById(courseId)
                                .orElseThrow(() -> new CourseNotFoundException("Course not found"));

                Boolean isUserAlreadyEnrolled = userCourseMappingRepo.existsMapping(currentUser.getId(),
                                course.getId());
                if (isUserAlreadyEnrolled) {
                        return CustomResponse.success("User already enrolled to the course", "Already enrolled",
                                        HttpStatus.OK);
                }
                userCourseMappingRepo.save(
                                UserCourseMappingEntity.builder().courseId(course.getId()).userId(currentUser.getId())
                                                .build());
                return CustomResponse.success("User successfully enrolled to the course", "Enrolled to the course",
                                HttpStatus.OK);
        }
}
