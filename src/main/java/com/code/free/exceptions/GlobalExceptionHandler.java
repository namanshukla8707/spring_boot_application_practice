...
public class GlobalExceptionHandler {
    ...
    public <T> ApiResult<T> handleAuthenticationException(AuthenticationException exception) {
        return CustomResponse.failure("Authentication failed: " + exception.getMessage(), "AUTHENTICATION_FAILED",
                HttpStatus.UNAUTHORIZED);
    }

    @ExceptionHandler(JwtException.class)
    public <T> ApiResult<T> handleJwtException(JwtException exception) {
        return CustomResponse.failure("Invalid JWT token: " + exception.getMessage(), "INVALID_JWT_TOKEN",
                HttpStatus.UNAUTHORIZED);
    }

    @ExceptionHandler(AccessDeniedException.class)
    public <T> ApiResult<T> handleAccessDeniedException(AccessDeniedException exception) {
        return CustomResponse.failure("Access denied: Insufficient permissions" + exception.getMessage(),
                "ACCESS_DENIED", HttpStatus.FORBIDDEN);
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    public <T> ApiResult<T> handleDataIntegrityViolationException(DataIntegrityViolationException exception) {
        return CustomResponse.failure("Data integrity violation: " + exception.getMessage(), "DATA_INTEGRITY_VIOLATION",
                HttpStatus.UNPROCESSABLE_ENTITY);
    }

    @ExceptionHandler(ConstraintViolationException.class)
    public <T> ApiResult<T> handleAttributesValidationException(ConstraintViolationException exception) {
        return CustomResponse.failure("Validation failed: " + exception.getMessage(), "ATTRIBUTE_VALIDATION_FAILED",
                HttpStatus.UNPROCESSABLE_ENTITY);
    }

    @ExceptionHandler(TransactionSystemException.class)
    public <T> ApiResult<T> handleTransaction(TransactionSystemException exception) {
        return CustomResponse.failure("Transaction failed: " + exception.getMessage(), "TRANSACTION_FAILED",
    ...
    public <T> ApiResult<T> handleJpaErrors(Exception ex) {
        return CustomResponse.failure("Database error: " + ex.getMessage(), "DATABASE_ERROR",
                HttpStatus.SERVICE_UNAVAILABLE);
    }

    @ExceptionHandler(Exception.class)
    public <T> ApiResult<T> handleGenericException(Exception exception) {
        return CustomResponse.failure("An unexpected error occurred: " + exception.getMessage(),
                "INTERNAL_SERVER_ERROR", HttpStatus.INTERNAL_SERVER_ERROR);
    }

    @ExceptionHandler(EmailNotFoundException.class)
    public <T> ApiResult<T> handleEmailNotFoundException(EmailNotFoundException exception) {
        return CustomResponse.failure(exception.getMessage(), exception.getError(), HttpStatus.NOT_FOUND);
    }
